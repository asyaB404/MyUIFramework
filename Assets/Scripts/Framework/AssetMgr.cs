...
public static class AssetMgr
{
    ...
    /// </summary>
    private static readonly Dictionary<AssetLoadType, Dictionary<string, AsyncOperationHandle>> LoadTypeToHandleDict =
        new();
    ...
    /// <returns></returns>
    public static async UniTask<T> LoadAssetAsync<T>(string path, AssetLoadType assetLoadType = AssetLoadType.Permanent)
        where T : Object
    {
        ...
        if (!LoadTypeToHandleDict.ContainsKey(assetLoadType))
        {
            LoadTypeToHandleDict.Add(assetLoadType, new Dictionary<string, AsyncOperationHandle>());
        }

        // 获取到目标类型的字典
        var dict = LoadTypeToHandleDict[assetLoadType];
    ...
    /// <param name="assetLoadType">资源加载类型，临时资源和永久资源</param>
    public static void LoadAssetAsync<T>(string path, Action<T> callBack,
    ...
    /// <param name="path">资源路径，如果为空则不会加载</param>
    /// <param name="assetLoadType">资源加载类型，临时资源和永久资源</param>
    public static T LoadAssetSync<T>(string path, AssetLoadType assetLoadType = AssetLoadType.Permanent)
        where T : Object
    {
        if (string.IsNullOrEmpty(path))
        {
    ...
    public static async UniTask<SceneInstance> LoadSceneAsync(string path,
    ...
    {
        foreach (var loadType in LoadTypeToHandleDict.Keys)
        {
            ReleaseByLoadType(loadType);
        }

        LoadTypeToHandleDict.Clear();
    }
